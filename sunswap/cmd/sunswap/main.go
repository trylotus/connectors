...
import (
	...
	"github.com/trylotus/connectors/sunswap"
	"github.com/trylotus/connectors/sunswap/factory"
	"github.com/trylotus/connectors/sunswap/pair"
...
func main() {
	...
	store, err := sunswap.NewStore(ctx, os.Getenv("DATA_SOURCE"))
	if err != nil {
		log.Fatal().Err(err).Str("source", os.Getenv("DATA_SOURCE")).Msg("Failed to create store")
	}

	source := sunswap.NewSource(client, store, FactoryContractAddr, sunswap.WithDefaultOptions())
