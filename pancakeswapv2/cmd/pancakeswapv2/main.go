...
import (
	...
	"github.com/trylotus/connectors/pancakeswapv2"
...
const FactoryContractAddr = "0xca143ce32fe78f1f7019d7d551a6402fc5350c73"
...
func main() {
	...
	store, err := pancakeswapv2.NewStore(ctx, os.Getenv("DATA_SOURCE"))
	if err != nil {
		log.Fatal().Err(err).Str("source", os.Getenv("DATA_SOURCE")).Msg("Failed to create store")
	}

	source := pancakeswapv2.NewSource(
		client,
		store,
		ethcommon.HexToAddress(FactoryContractAddr),
		pancakeswapv2.WithDefaultOptions(),
	)
